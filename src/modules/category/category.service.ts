...
export class CategoryService {
	...
	async create_category(createCategory: CreateCategoryDto) {
		...
		const category: any = this.categoryRepository.create(createCategory);
	...
	async get_category(id: string) {
		...
		if (!category) throw new NotFoundException('No se encontró la categoría.');
		return category;
	}
	...
	async update_category(id: string, editCategoryDto: EditCategoryDto) {
		...
		if (result.affected === 0) {
			throw new NotFoundException('No se encontró la categoria para actualizar');
		}
	...
	async create_subcategory(createSubcategory: CreateSubcategoryDto) {
		...
		const res = await this.subcategoryRepository.save(subcategory);
		return res;
	}
	...
	async update_subcategory(id: string, editSubcategoryDto: EditSubcategoryDto) {
		...
		if (result.affected === 0) {
			throw new NotFoundException('No se encontró la subcategoria para actualizar');
		}
