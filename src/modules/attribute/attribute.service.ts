...
export class AttributeService {
	...
	async get_attribute(id: string) {
		...
		if (!attribute) throw new NotFoundException('No se encontró el atributo.');
	...
	async update_attribute(id: string, editAttributeDto: EditAttributeDto) {
		try {
			...
			if (error.message.includes('ECONNREFUSED') || error.message.includes('failed to connect')) {
				throw new InternalServerErrorException('Perdida de conexión a internet.');
			}
			if (error.message.includes('ETIMEDOUT')) {
				throw new InternalServerErrorException('La conexión a la base de datos ha expirado.');
			}
			if (error.message.includes('ENOTFOUND')) {
				throw new InternalServerErrorException('No se encontró el host de la DB.');
			}
			if (error.message.includes('ECONNABORTED')) {
				throw new InternalServerErrorException('La solicitud se canceló.');
			}
			if (error.message.includes('NOTDOCUMENTUPDATE')) {
				throw new InternalServerErrorException('La categoría no pudo ser actualizada');
			}
			throw new InternalServerErrorException('Error actualizar la categoría.');
		}
	}
}
